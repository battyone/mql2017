//+------------------------------------------------------------------+
//|                                                    ea_AutoTL.mq5 |
//| ea_AutoTL v1.01                           Copyright 2017, fxborg |
...
input double InpSL=3.1; // Stop Loss distance
input double InpTP=10; // Take Profit distance
input double InpTS=3.0; // Trailing Stop distance
input double InpBE=  1; // Brake Even distance
input int    HourStart =   7; // Hour of trade start
input int    HourEnd   =  2; // Hour of trade end
input int    PositionExpire=20; // Position Expire
input string desc1="1--------- AutoTL  ------------";
input int InpFastPeriod=30;           // Fast Period
input int InpHiLoPeriod=70;           // HiLo Period
input string desc2="2.--------- TimeFrames -------------";
input ENUM_TIMEFRAMES InpTF1=PERIOD_M15; // Bars period
...
public:
   ...
   virtual bool      MyOpenPosition(long dir);              // open position on signal
...
//---
         ...
         if(dir==ORDER_TYPE_BUY)
           {
            if(m_buy_sig==2)
              {
               if(op-m_sl*m_pnt>=sl) m_trade.PositionClose(m_smb,1);
               m_buy_sig=3;
              }
           }
   ...
   if(!pos || (dir==ORDER_TYPE_BUY && m_buy_sig!=3))
     {
   ...
   if(!pos || (dir==ORDER_TYPE_SELL && m_sell_sig!=-3))
     {
   ...
   if(!pos)
     {
      if(m_buy_sig==1 && gann[1]==0 && gann[0]==1)
        {
         if(MyOpenPosition(ORDER_TYPE_BUY))
           {
            m_buy_sig=2;
           }
        }
      if(m_sell_sig==-1 && gann[1]==1 && gann[0]==0)
        {
         if(MyOpenPosition(ORDER_TYPE_SELL))
           {
            m_sell_sig=-2;
           }
...
//------------------------------------------------------------------	OpenPos
bool CMyEA::MyOpenPosition(long dir)
  {
   if(PositionSelect(m_smb)) return false;
   if(!CheckTime(StringToTime(IntegerToString(m_hourStart)+":00"),
      StringToTime(IntegerToString(m_hourEnd)+":00"))) return false;
